...
- Launch chat turns with native `--mode accept-edits` and the session vault as cwd, including resumed turns. Default edit review is auto-denied in headless agy 1.2.4 even for a file inside cwd. Never replace this with global permission changes or an all-tools bypass.
